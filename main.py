    ...
    while True:
        ...
        if option == '1':
            ...
            if not os.path.isfile('trained_neural_net'):  # check if neural_net file exists
                ...
                command='C:\\Program Files\\R\\R-3.6.3\\bin\\x64\\Rscript'
                path=os.getcwd()
                subprocess.run([command, 'getAttributes.r',path],shell=True)  # running R script for extracting data from recorded voice
